...
{ {
		# scope Docker secret variables to be wiped after initialization
		(
				### Load Secrets
				if [[ -d "/run/secrets/" ]]; then
						if [[ -f "/run/secrets/windscribe_server" ]]; then
								# load dotenv bash library
								source "$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )/dotenv"

								prefixWith "[DOCKER]" echo "Loading secrets from 'windscribe_server'"
								.env --file "/run/secrets/windscribe_server" export || {
										prefixWith "[DOTENV]" echo ".env secret file is invalidly formatted and failed to load." 1>&2
										exit 1
								}
						else
								prefixWith "[DOCKER]" echo "Secrets must be specified with the name 'windscribe_server' (detected secrets: $(ls /run/secrets))" 1>&2
						fi
				else
						prefixWith "[DOCKER]" echo "No configured secrets detected; secrets can be specified with an .env file under secret name 'windscribe_server'"
				fi
				...
				if [[ -z "${WINDSCRIBE_USERNAME}" ]]; then
						prefixWith "[WINDSCRIBE]" echo "Unset Windscribe username; ensure that the variable \$WINDSCRIBE_USERNAME is set properly" 1>&2
						exit 1
				fi
				if [[ "${WINDSCRIBE_USERNAME}" =~ [^a-zA-Z0-9_] ]]; then
						prefixWith "[WINDSCRIBE]" echo "Windscribe username must be alphanumeric (underscores allowed); ensure that the variable \$WINDSCRIBE_USERNAME is set properly" 1>&2
						exit 1
				fi
				# validate password checks
				if [[ -z "${WINDSCRIBE_PASSWORD}" ]]; then
						prefixWith "[WINDSCRIBE]" echo "Unset Windscribe password; ensure that the variable \$WINDSCRIBE_PASSWORD is set properly" 1>&2
						exit 1
				fi
				# ensure no newlines present (source: https://unix.stackexchange.com/a/276836)
				NL='
				'
				case "${WINDSCRIBE_PASSWORD}" in *"${NL}"*) prefixWith "[WINDSCRIBE]" echo "Windscribe password cannot contain new lines; ensure that the variable \$WINDSCRIBE_PASSWORD is set properly" 1>&2
						exit 1;;
				esac
				# validate windscribe location
				# ensure no newlines present
				case "${WINDSCRIBE_LOCATION}" in *"${NL}"*) prefixWith "[WINDSCRIBE]" echo "Windscribe location cannot contain new lines; ensure that the variable \$WINDSCRIBE_LOCATION is set properly" 1>&2
						exit 1;;
				esac


				### Prepare Dante Accounts
				# indicates that proxy server authentication is enabled
				authentication=false
				prefixWith "[SOCKS5]" echo "Searching for SOCKS_USERNAME and SOCKS_PASSWORD variables"
				...
				for socks_username in $(printenv | grep "^SOCKS_USERNAME"); do
						...
						if [[ "${username}" ]]; then
								if [[ "${username}" =~ [^a-zA-Z0-9_] ]]; then
										prefixWith "[SOCKS5]" echo "Detected user variable from \$SOCKS_USERNAME${suffix}, but the value was not alphanumeric (with _); username: ${username}" 1>&2
										exit 1
								fi
								if [[ "${password}" ]]; then
										prefixWith "[SOCKS5]" echo "Detected user variable from \$SOCKS_USERNAME${suffix} (username: ${username}); creating account"
								else
										prefixWith "[SOCKS5]" echo "Detected user variable from \$SOCKS_USERNAME${suffix}, but no password was specified at \$SOCKS_PASSWORD${suffix}; username: ${username}" 1>&2
										exit 1
								fi
						else
								prefixWith "[SOCKS5]" echo "Detected user variable from \$SOCKS_USERNAME${suffix}, but the value was empty; ignoring entry" 1>&2
								continue
						fi
				...
				if [[ "${authentication}" == false ]]; then
						prefixWith "[SOCKS5]" echo "No non-empty SOCKS5 accounts were received (check that the variables are set correctly)"
				fi
				...
				prefixWith "[RESOLV] config: " cat /etc/resolv.conf
